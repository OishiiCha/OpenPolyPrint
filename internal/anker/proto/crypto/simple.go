package crypto

// PPPPSimpleSeed is the seed for the simple (lib32100 variant) cipher.
var PPPPSimpleSeed = []byte("SSD@cs2-network.")

// PPPPSimpleShuffle is the 256-byte lookup table for the simple cipher.
var PPPPSimpleShuffle = [256]byte{
	,
}

// simpleHash computes a 4-byte hash from the seed.
func simpleHash(seed []byte) [4]byte {
	var hash [4]byte
	for _, b := range seed {
		hash[0] ^= b
		hash[1] += b / 3
		hash[2] -= b
		hash[3] += b
	}
	// Reverse: return hash[::-1]
	return [4]byte{hash[3], hash[2], hash[1], hash[0]}
}

// simpleLookup looks up a value in the shuffle table.
func simpleLookup(hash *[4]byte, b byte) byte {
	index := int(hash[b&0x3]) + int(b)
	return PPPPSimpleShuffle[index%len(PPPPSimpleShuffle)]
}

// SimpleDecrypt decrypts input using the simple cipher with the given seed.
func SimpleDecrypt(seed, input []byte) []byte {
	hash := simpleHash(seed)
	output := make([]byte, len(input))

	output[0] = input[0] ^ simpleLookup(&hash, 0)
	for i := 1; i < len(input); i++ {
		output[i] = input[i] ^ simpleLookup(&hash, input[i-1])
	}
	return output
}

// SimpleEncrypt encrypts input using the simple cipher with the given seed.
func SimpleEncrypt(seed, input []byte) []byte {
	hash := simpleHash(seed)
	output := make([]byte, len(input))

	output[0] = input[0] ^ simpleLookup(&hash, 0)
	for i := 1; i < len(input); i++ {
		output[i] = input[i] ^ simpleLookup(&hash, output[i-1])
	}
	return output
}

// SimpleDecryptString decrypts input using the default PPPP simple seed.
func SimpleDecryptString(input []byte) []byte {
	return SimpleDecrypt(PPPPSimpleSeed, input)
}

// SimpleEncryptString encrypts input using the default PPPP simple seed.
func SimpleEncryptString(input []byte) []byte {
	return SimpleEncrypt(PPPPSimpleSeed, input)
}
